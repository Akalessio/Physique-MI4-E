...
dt = 1E-7 #dérivée selon le temps
dx = 0.001 #dérivée selon l'espace
nx = int(1 / dx) * 2 #nombre de point d'espace
nt = 50000 #nombre de point temporel
n_frame = int(nt / 1000) + 1 #nombre d'image
s = dt / (dx ** 2) #????
v0 = -4000  # Potentiel
e = 2
E = e * v0 #Energie
...
wp_gauss = normalisation * np.exp(1j * k * o - ((o - xc) ** 2) / (2 * (sigma ** 2)))
densite = np.zeros((nt, nx))
densite[0, :] = np.abs(wp_gauss[:]) ** 2
...
ani = animation.FuncAnimation(fig, animate, init_func=init, frames=int(nt / 1000) + 1, blit=False, interval=100, repeat=False)
...
for i in range(n_states):
    if energies[i] > 0:
        print("Etat non lié n =", i + 1, ",E =", energies[i])
        continue
